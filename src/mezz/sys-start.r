...
start: func [
	...
	if all [
		not quiet
		any [script do-arg flags/verbose flags/usage flags/help]
	][
	...
	either file? script [
		...
		either exists? second script-path [
			...
			either 'module = select first code 'type [
				...
				if object? tmp: do-needs/module first code [append code tmp]
				import catch/quit [make module! code]
			][
				do-needs first+ code
				return catch/quit intern code
			]
